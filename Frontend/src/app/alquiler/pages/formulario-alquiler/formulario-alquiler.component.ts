...
export class FormularioAlquilerComponent {
  ...
  registrarAlquiler() {
    ...
    if(this.formularioAlquiler.valid) {
      ...
      this.alquilerService.createAlquiler(alquiler).subscribe(
        result => {
          if(result.status == 1) {
            this.router.navigate(['/dashboard/listado-alquiler']);
  ...
  modificarAlquiler() {
    if(this.formularioAlquiler.valid) {
      ...
      this.alquilerService.updateAlquiler(alquiler).subscribe(
        ...
        error => {
          if (error.error && error.error.msg === 'Ya existe un alquiler activo para este local') {
            this.toastr.error("No se puede activar el alquiler porque ya existe un alquiler activo para este local", "Error");
          } else {
  ...
  getInquilinos() {    
    this.usuarioService.getByPerfil("inquilino").subscribe(
      result => {
